...
namespace Mule_Plugin\Includes\Media;
...
class Media {
	...
	private function dependencies() {
		...
		include_once MULE_PATH . 'includes/media/class-svg-support.php';

		// Replace WP gallery shortcode if Fancybox option is used.
		$fancybox = get_option( 'mule_enqueue_fancybox_script' );

		if ( $fancybox ) {
			require_once MULE_PATH . 'includes/media/class-gallery-shortcode.php';
		}
	...
	public function image_sizes() {
		...
		add_image_size( __( 'Meta Image', 'mule-plugin' ), 1200, 630, true );
		 ...
		 */
		add_image_size( __( 'Column Thumbnail', 'mule-plugin' ), 48, 48, true );
	...
	public function rss_featured_images( $content ) {
		...
		$size = apply_filters( 'mule_rss_featured_image_size', 'medium' );
