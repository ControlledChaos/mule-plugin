...
class Meta_Description {
	...
	public function __construct() {
		...
		add_action( 'mule_meta_description_tag', [ $this, 'description' ] );
	...
	public function description() {
		...
		if ( mule_acf_options() ) {
			 ...
			 */
			$acf_blog_desc = get_field( 'mule_meta_blog_description', 'option' );
			...
			$wp_blog_desc = get_option( 'mule_meta_blog_description' );
		...
		$search_desc      = wp_strip_all_tags(
			sprintf(
				'%1s \'%2s\'',
				__( 'Showing results for', 'mule-plugin' ),
				$search_query
			)
		);

		// Apply a filter to hard-coded text.
		$search_meta_desc = apply_filters( 'mule_meta_description_search', $search_desc );
		...
		$meta_description = apply_filters( 'mule_meta_description', $description );
 ...
 */
function mule_meta_description() {
