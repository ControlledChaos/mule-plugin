...
$tabs = [
    ...
    sprintf(
        ...
        esc_html__( 'General', 'mule-plugin' )
    ),
    ...
    sprintf(
        ...
        esc_html__( 'Vendor', 'mule-plugin' )
    )

];

// Apply a filter to the tabs array for adding tabs.
$page_tabs = apply_filters( 'mule_tabs_script_options', $tabs );
...
if ( 'general' == $active_tab ) {
    $section = 'mule-scripts-general';
    $fields  = 'mule-scripts-general';
} elseif ( 'vendor' == $active_tab ) {
    $section = 'mule-scripts-vendor';
    $fields  = 'mule-scripts-vendor';
} else {
    $section = null;
    $fields  = null;
}
...
$do_section = apply_filters( 'mule_section_script_options', $section );
$do_fields  = apply_filters( 'mule_fields_script_options', $fields );
 ...
 */
if ( 'general' == $active_tab  ) {
    $save = __( 'Save General', 'mule-plugin' );
} elseif ( 'vendor' == $active_tab ) {
    $save = __( 'Save Vendor', 'mule-plugin' );
} else {
    $save = __( 'Save Settings', 'mule-plugin' );
}

// Apply a filter for new tabs added by another plugin or from a theme.
$button = apply_filters( 'mule_save_script_options', $save );

?>
<div class="wrap">
    <h1 class="wp-heading-inline"><?php esc_html_e( 'Script Options', 'mule-plugin' ); ?></h1>
    <?php if ( is_rtl() ) : ?>
    <p class="description"><?php esc_html_e( 'Script settings from the Mule plugin. More information in the Help tab at upper left.', 'mule-plugin' ); ?></p>
    <?php else : ?>
    <p class="description"><?php esc_html_e( 'Script settings from the Mule plugin. More information in the Help tab at upper right.', 'mule-plugin' ); ?></p>
    <?php endif; ?>
