...
class Meta_SEO_Callbacks {
	...
	public function disable_meta( $args ) {
		...
		$option = get_option( 'mule_meta_disable' );

		$html = '<p><input type="checkbox" id="mule_meta_disable" name="mule_meta_disable" value="1" ' . checked( 1, $option, false ) . '/>';

		$html .= '<label for="mule_meta_disable"> '  . $args[0] . '</label></p>';
	...
	public function  schema_org_type( $args ) {
		...
		// Get the field file.
		include_once MULE_PATH . 'admin/partials/field-callbacks/schema-org-type.php';
 ...
 */
function mule_meta_seo_callbacks() {
