...
class Welcome {
    ...
    public function __construct() {
		...
		if ( mule_acf_options() ) {
			$dismiss = get_field( 'mule_remove_welcome_dismiss', 'option' );

		// If ACF is not active, get the field from the WordPress/ClassicPress options page.
		} else {
			$dismiss = get_option( 'mule_remove_welcome_dismiss' );
		}
		...
		// If ACF is active, get the field from the ACF options page.
		if ( mule_acf_options() ) {
			$welcome = get_field( 'mule_custom_welcome', 'option' );
		} else {
	...
	public function widget_areas() {
		...
		register_sidebar( [
			'name'          => __( 'Welcome Panel - First Area', 'mule-plugin' ),
			'id'            => 'mule_welcome_widget_first',
			'description'   => __( '', 'mule-plugin' ),
		...
		register_sidebar( [
			'name'          => __( 'Welcome Panel - Second Area', 'mule-plugin' ),
			'id'            => 'mule_welcome_widget_second',
			'description'   => __( '', 'mule-plugin' ),
			'before_widget' => '<div id="%1$s" class="widget %2$s">',
			'after_widget'  => '</div>',
			'before_title'  => '<h3>',
		...
		register_sidebar( [
			'name'          => __( 'Welcome Panel - Third Area', 'mule-plugin' ),
			'id'            => 'mule_welcome_widget_last',
			'description'   => __( '', 'mule-plugin' ),
			'before_widget' => '<div id="%1$s" class="widget %2$s">',
			'after_widget'  => '</div>',
			'before_title'  => '<h3>',
	...
	public function welcome_panel() {
		...
		if ( ! empty( $welcome ) ) {
			...
			include_once MULE_PATH . 'admin/dashboard/partials/welcome-panel.php';
		}
	...
	public function styles() {
        ...
        if ( $screen->id == 'dashboard' ) {
            wp_enqueue_style( MULE_ADMIN_SLUG . '-welcome', MULE_URL .  'assets/css/welcome.min.css', [], null, 'screen' );
        }
